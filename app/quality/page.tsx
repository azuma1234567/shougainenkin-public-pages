...
const DESCRIPTION =
  `出典の決まり、数字の決まり、引用の決まり、機械で検査していること、書かないこと、そして訂正の記録。${SITE_NAME}の記事と機能が、どう作られ、どう直されているかを書いています。`;
...
export default function QualityPage() {
  return (
    <>
      ...
      <h2>数字の決まり</h2>
      <p>
        金額(年金額・加算・所得制限の線・扶養の線)は、サイトの中の1か所で管理しています。記事も機能も、そこから同じ値を引きます。ページごとに違う数字が出ることを、仕組みで防いでいます。
      </p>
      <p>
      ...
      <ul>
        ...
        <li>機能の中に、入力を送信するコード(fetch・XMLHttpRequest・sendBeacon・WebSocket・フォーム送信)が無いこと</li>
        <li>「あなたは◯級」「◯級相当」「通りそう」「もらえそう」のような、判定や予測に読める言葉が無いこと</li>
        <li>サイト内のリンクが切れていないこと、各ページに本文から2本以上のリンクが届いていること</li>
        <li>申立書の機能が、公式様式の座標どおりに印字し、紙に色が付かないこと</li>
      </ul>
