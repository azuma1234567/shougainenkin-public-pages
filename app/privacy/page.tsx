...
export default function PrivacyPage() {
  return (
    <>
      ...
      <ul>
        <li>閲覧したページの URL とページの題名、参照元のURL</li>
        <li>閲覧日時、サイト内での滞在時間や操作の回数</li>
        <li>ブラウザとOSの種類、画面の大きさ、言語設定</li>
        <li>IPアドレスから推定されるおおよその地域(市区町村まで。IPアドレス自体は保存しません)</li>
        <li>Google アナリティクスが発行する識別子(Cookie)</li>
      </ul>
      <p>
        送信先は Google LLC で、利用目的は本サイトの利用状況の把握と、記事や導線の改善です。
        広告の配信や、広告のための個人の特定には使いません(広告関連の Cookie は既定で無効にしています)。
      </p>
      <ul>
        <li>
          Google のプライバシーポリシー:{" "}
      ...
      <p>
        ...
        停止すると、Google アナリティクスのタグを読み込まず、通信も行いません。すでに保存されている Cookie も削除します。
