...
import { hubDescription } from "@/lib/hub-jsonld";
...
export function hubMetadata(hub: HubDefinition): Metadata {
  ...
  /* description は Article.description(lib/hub-jsonld.ts)と同じ文 */
  return pageMetadata({ title: content?.metaTitle ?? content?.title ?? hub.label, description: hubDescription(hub, content), path: hub.path });
}
