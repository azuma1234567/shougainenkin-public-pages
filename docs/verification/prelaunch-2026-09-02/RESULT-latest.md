...
origin: http://localhost:3200 / ページ数: 162 / 基準: git main
...
| A-3 | ○ | 404が出るリンクがゼロ | リンク先 162 件を検査、破損 0 |
| A-4 | ○ | 未公開ページへのリンクがゼロ | 予約slug 4 件へのリンク 0 |
| A-5 | ○ | noindex が残っていない | noindex 0 / 162 |
| A-6 | ○ | canonical が自分自身を指している | 自己参照 162 / 162 |
...
| # | 判定 | 項目 | 件数 |
|---|---|---|---|
| B-1 | ○ | 孤立ページがゼロ(本文からの内部リンクが最低1本) | 孤立 0 / 162(除外 5)<br>※ヘッダー・フッターのリンクは数えない。法務・案内ページ(about/privacy/terms/quality/support)はフッターのみで可 |
...
| B-8 | ○ | 構造化データが妥当 | 型別件数 WebSite:1 BreadcrumbList:160 undefined:48 HowTo:2 FAQPage:47 DefinedTermSet:1 Article:47 ItemList:1、エラー 0<br>※スキーマ必須項目の静的検査。Google のリッチリザルトテストは公開URLで別途実施 |
...
| # | 判定 | 項目 | 件数 |
|---|---|---|---|
| C-1 | ○ | sitemap.xml: 全公開ページが入り、未公開ページが入っていない | 収録 162、リンクはあるが未収録 0、予約slugの混入 0、200以外 0 |
| C-2 | 手動 | sitemapの分割 | 対象外(162ページ・単一 sitemap.xml で十分。50,000 URL 超で再検討)<br>※Google の分割要件は 50,000 URL または 50MB。2026-09-02 に対象外とした |
| C-3 | 手動 | Search Console にサイトマップを送信 | 手動作業(スクリプト対象外)<br>※sitemap.xml を送る |
| C-4 | 手動 | 主要10ページの URL検査(インデックス登録リクエスト) | 10ページとも200: ○(送信自体は手動) |
| C-5 | ○ | 旧URLからのリダイレクトが必要な変更がない | 公開前URL 71 件のうち200以外 0<br>※main ブランチの sitemap 静的ページ + 記事URL |
