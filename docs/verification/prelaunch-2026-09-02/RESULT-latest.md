...
| A-2 | ○ | 既存記事の本文が消えていない(公開前の8割未満・本文なしを×) | 本文が消えた/8割未満 0、方針上の削除で減ったもの 0(いずれも8割以上を維持)、47 記事を比較<br>※main ブランチの本文文字列と比較。減少分は docs/verification/prelaunch-2026-09-02/RESULT.md 末尾に一覧 |
...
| B-3 | × | 500字未満のページの一覧 | 500字未満 1(うち誤解カード 0)。実例の個別ページ 0 件<br>※実例94件の個別ページはサイトマップに無い(未実装) |
...
| C-6 | ○ | sitemap の lastModified が中身より古くない | 静的 27 件・ハブ 44 件を「中身が最後に変わった日」と突き合わせ、古いもの 0<br>※ページを変えたら lib/sitemap-static-dates.ts と data/hubs/*.json の dateModified も同じ PR で直す |
