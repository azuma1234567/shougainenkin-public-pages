...
origin: http://localhost:3107 / ページ数: 166 / 基準: git main
...
| A-2 | ○ | 既存記事の本文が消えていない(公開前の8割未満・本文なしを×) | 本文が消えた/8割未満 0、方針上の削除で減ったもの 2(いずれも8割以上を維持)、46 記事を比較<br>※main ブランチの本文文字列と比較。減少分は docs/verification/prelaunch-2026-09-02/RESULT.md 末尾に一覧 |
...
| A-8 | ○ | 金額が全ページで一致している(data/amounts.ts から導出できる) | 10万円以上の金額 28 種のうち説明済み 28、未説明 0(参考: 直書きのあるファイル 34)<br>※説明済みの式は末尾の付記に全件 |
...
| B-8 | ○ | 構造化データが妥当 | 型別件数 WebSite:1 BreadcrumbList:165 undefined:97 HowTo:2 FAQPage:96 Person:1 Organization:1 MobileApplication:1 DefinedTermSet:1 Article:95 ItemList:1、エラー 0<br>※スキーマ必須項目の静的検査。Google のリッチリザルトテストは公開URLで別途実施 |
...
- /jitsurei (99本・宣言済み: 裁決事例集。裁決を引用した記事・誤解カードが文末で戻す導線)
...
| URL | タイトル | 文字数 |
|---|---|---|
| /columns/moushitatesho-a4-insatsu | 障害年金の申立書をA4で印刷する方法｜PDF・コンビニ印刷も解説｜障害年金申請サポート | 11010 |
| /columns/moushitatesho-kikan-kugiri | 障害年金の申立書は期間をどう区切る？通院・就労・症状変化の書き方｜障害年金申請サポート | 11672 |
| /columns/teishutsusaki-yuusou | 障害年金の書類はどこに提出する?｜年金事務所・市役所・郵送｜障害年金申請サポート | 9746 |

## A-8 付記: 金額の検算(data/amounts.ts からの導出)

- 説明済み: 111,240円 = (basicGrade2(847,300) + childFirstSecond(243,800) + childFirstSecond(243,800)) ÷ 12 ≒ 111,240
...
- 説明済み: 211,825円 = (basicGrade2(847,300) + basicGrade2(847,300) + basicGrade2(847,300)) ÷ 12 ≒ 211,825
...
参考: amounts.ts の値を直書きしているファイル 34 件(判定には使わない): components/ApplicationFlowPage.tsx (14), components/platform/ShinseiRestyled.tsx (4), content/columns/fushikyuu-shinsa-seikyu.ts (2), content/columns/gaku-kaitei-seikyuu.ts (2), content/columns/hatachi-mae.ts (14), content/columns/hatarakinagara.ts (4), content/columns/hattatsu-shougai.ts (4), content/columns/hikazei-shuunyuu.ts (23), content/columns/hitsuyou-shorui-seishin.ts (3), content/columns/ikura-moraeru.ts (78), content/columns/jukyuugo-tetsuduki.ts (26), content/columns/kiso-kousei-chigai.ts (16), content/columns/ninteibi-jigojusho.ts (11), content/columns/nofu-yoken.ts (6), content/columns/shikyuu-teishi-fukkatsu.ts (4), content/columns/shinsei-kikan.ts (5), content/columns/shoubyou-teatekin.ts (7), content/columns/shougaisha-koyou-nenkin.ts (4), content/columns/sokyuu-seikyuu.ts (2), content/columns/techou-to-nenkin.ts (7), content/columns/tokyu-hantei-guideline.ts (2), lib/columns.ts (4), lib/hub-index.tsx (6), data/gokai-bodies.ts (83), data/gokai.ts (20), data/hubs/byoki-chiteki.json (2), data/hubs/byoki-jinzou-touseki.json (1), data/hubs/byoki-shinzou.json (3), data/hubs/byoki-shitai.json (2), data/hubs/byoki-tounyou.json (1), data/hubs/byoki-utsu-soukyoku.json (2), data/hubs/joukyou-hatachi-mae.json (4), data/hubs/joukyou-hitorigurashi.json (1), data/hubs/okane-ikura.json (14)

## 参考: 公開前より本文が減った既存記事(8割以上は維持)

- /columns/jushinjokyo-shomeisho (14127 → 13585字, 96%)
- /columns/moushitatesho-a4-insatsu (11568 → 11346字, 98%)
