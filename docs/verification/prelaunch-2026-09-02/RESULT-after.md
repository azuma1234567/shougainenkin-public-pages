# 公開前チェック 再実行結果 (2026-09-02, §1〜§8 適用後 / HEAD 9f8b7f4)

origin: http://localhost:3106 / ページ数: 158 / 基準: git main
...
| A-3 | × | 404が出るリンクがゼロ | リンク先 159 件を検査、破損 1 |
| A-4 | ○ | 未公開ページへのリンクがゼロ | 予約slug 4 件へのリンク 0 |
| A-5 | ○ | noindex が残っていない | noindex 0 / 158 |
| A-6 | ○ | canonical が自分自身を指している | 自己参照 158 / 158 |
...
- /dougu/moushitatesho (404) ← /dougu

## B

| # | 判定 | 項目 | 件数 |
|---|---|---|---|
| B-1 | × | 孤立ページがゼロ(本文からの内部リンクが最低1本) | 孤立 1 / 158(除外 5)<br>※ヘッダー・フッターのリンクは数えない。法務・案内ページ(about/privacy/terms/quality/support)はフッターのみで可 |
| B-2 | ○ | 被内部リンクが50本を超えるページがゼロ | 50本超 0<br>※パンくずと誤解カードの「一覧へ戻る」由来のリンクは数えない |
| B-3 | × | 500字未満のページの一覧 | 500字未満 4(うち誤解カード 0)。実例の個別ページ 0 件<br>※実例94件の個別ページはサイトマップに無い(未実装) |
...
| B-8 | ○ | 構造化データが妥当 | 型別件数 WebSite:1 BreadcrumbList:157 undefined:48 HowTo:2 FAQPage:47 DefinedTermSet:1 Article:47 ItemList:1、エラー 0<br>※スキーマ必須項目の静的検査。Google のリッチリザルトテストは公開URLで別途実施 |
...
| B-10 | × | 更新日が全ページに表示されている | 更新日/確認日の表示なし 1 |
...
| # | 判定 | 項目 | 件数 |
|---|---|---|---|
| C-1 | × | sitemap.xml: 全公開ページが入り、未公開ページが入っていない | 収録 158、リンクはあるが未収録 1、予約slugの混入 0、200以外 0 |
| C-2 | 手動 | sitemapの分割 | 対象外(158ページ・単一 sitemap.xml で十分。50,000 URL 超で再検討)<br>※Google の分割要件は 50,000 URL または 50MB。2026-09-02 に対象外とした |
...
| 項目 | 期待 | 結果 | 差の内容 |
|---|---|---|---|
| A-1〜A-10 | すべて○ | **A-3 のみ×** | `/dougu` から `/dougu/moushitatesho`(404)へのリンク。`/dougu/*` は別の指示書(`docs/claude-code-orders-dougu-2026-09-02.md`、アプリ側 repo)で扱う道具ページで、この作業の対象外。一覧ページ(`f2a3408`)が先に入り、個別ページが未実装。**直していない** |
| A-8 | 説明済み18・未説明0 | ○ 説明済み23・未説明0 | §4 で索引に載せた額と §7 の「数字で見ると」の額が加わり23種。すべて `data/amounts.ts` から導出できた |
| B-1 | ○(孤立0・除外5) | **×** 孤立1(除外5) | 孤立は `/dougu`(上と同じ理由)。`/byoki/shikaku` は解消 |
| B-2 | ○ | ○ | パンくず由来を除いた結果、50本超は0 |
| B-3 | `/gokai/` 0・索引0、注記のみ | **×** 4件 | 誤解カードは48枚とも500字以上。残りは `/nayami` 450字・`/okane` 464字(§4 の指定文をそのまま入れても届かない。文言は指示どおりで、自分で足していない)、`/about` 493字、`/dougu` 368字(対象外) |
| B-4〜B-8 | ○のまま | ○ | — |
| B-9 | ○(なし0・二重0) | ○ | `/columns` と `/dougu` は自前の BreadcrumbList があったため、部品側を `jsonLd={false}` にして二重を解消(`9f8b7f4`) |
| B-10 | ○ | **×** 1件 | `/dougu`(対象外) |
| C-1 | ○(157+変化なし) | **×** | 収録158(`/dougu` が加わった)。未収録1は `/dougu/moushitatesho`(存在しないページへのリンク。上と同じ) |
| C-2 | 対象外 | 対象外 | — |
| C-4 / C-5 | ○のまま | 手動 / ○ | — |

**判断が要るもの(直していない)**
1. `/dougu` まわりの4件(A-3・B-1・B-10・C-1)は、道具ページの個別実装が入るか、一覧からのリンクを外すかで解消する。この作業の範囲外。
2. `/nayami`・`/okane` の本文は指定の文面をそのまま入れても 500 字に届かない(450 / 464)。文を足すなら文面の指示が要る。
3. `/about` は 493 字(更新日の表示で +14 字)。

**目視(§7 完了条件)**: `/gokai/oya-no-shuunyuu` を375pxで表示し、「自分の場合を確かめる」(3項目)・「窓口で聞く一言」・「数字で見ると」の3ブロックが順に出ることを確認。本文554字、横あふれなし。
