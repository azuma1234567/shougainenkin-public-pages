...
## 2026-09-04 仕上げ指示§1〜§4の対応（最新判定）

上記は仕上げ前の記録としてそのまま保存する。ユーザーの追加指示を受けて検査条件とkuriageの指定1段落のみを修正した。**最新の判定は12項目すべて○、公開前チェックA-8・B-2も○**。

### 変更内容

- §1: kuriage「数字で見ると」の第1段落だけを指定文言に差し替え、生成データを更新。他の段落・他の47枚は変更なし。`git diff main -- docs/gokai/` は当該1段落だけ。
- §1: amounts-deriveにbasicGrade2/basicGrade2Oldのみの0.4%×1〜60か月の繰上げ減額を追加。「繰上げ」「繰り上げ」が段落にある場合のみ採用。円未満は整数比で切り捨て、0.5%の式は追加していない。
- §2: B-2に理由つきのLINK_HUBSを宣言。/jitsureiのみ上限判定の対象外とし、件数・理由は毎回付記。未宣言ページの閾値50本は変更なし。
- §3: 検査11は3記事の`<main ...>`から`</main>`までの生文字列のSHA-256を比較。HTML全体やDOM再シリアライズのハッシュにはしない。比較元は変更前の5e5c897を別worktreeでビルドして取得し、baseline.jsonへ追加した。元の全HTMLハッシュ記録も残している。
- 保護対象のapp/columns/・content/columns/・data/gokai.ts、一覧・ハブ・OG・CSSは引き続き差分0。原稿検査は許可された1段落の完全一致置換だけを例外とし、その他の変更を禁止する。

### 検算とハブ件数

`847,300 × (1 − 0.004 × 60) = 643,948`
...
|---|---|---|
| 1 | ○ | 48件・slug一致・生成物再現性 |
| 2 | ○ | title・description・確認日・1,500字以上 |
| 3 | ○ | check/ask完全一致 |
| 4 | ○ | 全内部リンク公開済み |
| 5 | ○ | 裁決ID全52件が存在・verified |
| 6 | ○ | 未説明額0。643,948円も上記の式で説明 |
| 7 | ○ | 禁止文字列なし |
| 8 | ○ | 実例節のcaseと末尾リンク |
| 9 | ○ | 全48枚のArticle/FAQPage・メタ情報 |
| 10 | ○ | 原稿の見出し順と配信本文が一致 |
| 11 | ○ | 保護対象3記事のmainハッシュがすべて作業前と一致、保護対象ソース差分0 |
| 12 | ○ | ページ数166、作業前と判定一致。A-8/B-2○、既存B-1/B-3/B-10のみ×を維持 |

3記事のmain SHA-256（変更前後で同一）:

```text
moushitatesho-a4-insatsu     9ee6e931e1e88182d4f173945f37e754c86a30aac0eed04ed33e362e6e9b2fd5
moushitatesho-kikan-kugiri  4c95385b9e099f6fb95579b556d44c979fb204aefa2c183679531aa44568dc93
teishutsusaki-yuusou        4705052fde30349cd62906115e590095600b4940dd1be6018161bec9c163d483
```

### 実行結果

- importスクリプトの再生成と`--check`: exit 0。
- クリーンbuild: exit 0。環境が`rm -rf`を拒否したため、既存.nextを一時ディレクトリへ退避し、.nextが存在しない状態で`npm run build`を実行した。ビルド中のフォント取得は一時的にECONNRESETで再試行されたが、最終的に成功。
- `node scripts/verify-early-pension-amounts.mjs`: exit 0。2キー×1〜60か月、端数、文脈ゲート、範囲外・他キー・0.5%除外を検査。
- `npm run prelaunch:check -- http://localhost:3195 RESULT-gokai-bodies-shiage.md`: exit 0。A全項目○、B-2○、C-1○、166ページ。
- `VERIFY_BUILT=1 VERIFY_INTEGRATION=1 npm run verify:gokai`: exit 0。
- `VERIFY_ORIGIN=http://localhost:3195 VERIFY_BUILT=1 npm run verify:gokai`: exit 0。実URLの一覧・カテゴリ・OG・ハブ・48枚の配信本文も検査。
- [仕上げ後の公開前チェック全文](../prelaunch-2026-09-02/RESULT-gokai-bodies-shiage.md)

仕上げコミットの件名: `fix(prelaunch): 繰上げ減額を金額の説明に加え、実例ハブを被リンク上限の対象外にする`。この検証済み状態をmainにマージ・pushする。
