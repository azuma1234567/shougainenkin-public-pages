...
- サイト内検索は後続指示で実機能化。既存47コラムと主要5ハブを検索対象にし、URLで検索語を復元できる
...
- 社労士掲載枠は実データがないため既定OFF。検索は後続指示に基づき実機能化した。
...
トップ検索は「初診日 カルテ」「うつ病 働きながら」「不支給」、該当なし、Enter送信、URLからの復元、検索結果から記事への遷移を確認した。375pxでは入力欄・ボタン・結果パネルが画面内に収まり、横スクロールなし。
