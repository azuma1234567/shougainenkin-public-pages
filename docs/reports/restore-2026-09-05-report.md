...
### (1) docs の未コミット変更 → `da1407b` docs: 2026-09-05 のモック・指示書・検証記録

`git stash pop` は最初「stash を残したまま」終わった(適用されていなかった)ので `git stash apply` で当て直し、docs だけを 1 コミットにして stash を捨てた。コードには触れていない(docs 以外の変更 0)。混ざっていた `__pycache__/*.pyc` 1 件はコミットから外した(追跡しない)。

### (2) prelaunch C-6 → `58083b8` + `e502810`

- `58083b8` 指示の 4 件: `/hajimete` `/jitsurei` `/suuji`(lib/sitemap-static-dates.ts)と `data/hubs/joukyou-65sai-ijou.json` を 2026-09-05 に。
- `e502810` 同じ理由で C-6 に残った 3 本: `data/hubs/joukyou-hatarakinagara.json` `nayami-koushin.json` `nayami-shikyuu-teishi.json`(復元コミットが幹10 の入口の行を戻したもの)。**指示の 4 件に無いが、同じ直し方で足した。**不要なら `e502810` だけ戻せる。
- 結果: **C-6 ○**。B-3・B-10 はそのまま ×(ログ: docs/verification/restore-2026-09-05/prelaunch-after-dates.txt)。

### (3) `app/suuji/page.tsx:113` の文 — **直していない(報告のみ)**

文: 「「精神疾患で申請していいのか」というためらいは、この数字と逆です。10人のうち7人が精神の診断書で申請しています。」

何が誤りか:
- 「7割」の元は `mentalShare` = 精神障害・知的障害の診断書による**新規裁定(決定)件数** ÷ 新規裁定の決定件数(令和6年度 業務統計、70.3%)。分母は「決まった件数」であって「申請した人」ではない(取下げなどで申請件数 ≠ 決定件数)。
- 単位も「件」であって「人」ではない(同じ人が複数の診断書を出すことがあり、直前の注記でもそう書いている)。
- つまり数字(7/10 ≒ 70.3%)は合っていて、誤っているのは「申請しています」「10人」という**言い方**。

8026dd6 がトップでどう直したか: 「10人のうち7人が精神の診断書で申請しています。」→「新しく決まった障害年金の10件のうち7件は、精神の障害です。」(主語を「申請した人」から「新しく決まった件」に、単位を人から件に変えた。数字は変えていない)。

なぜ直さなかったか: 今回の指示は「数字と出典だけを同じ直し方で直し、文そのものは変えない」。この文の誤りは数字ではなく言い方にあり、文を変えずに直す方法が無い(数字は正しく、出典はすぐ上のタイル「新規裁定の約70%」が業務統計を出典にしている)。誤った言い方の文に出典を添えるのはかえって良くないので、「迷うなら直さず報告だけ」に従った。直すなら 8026dd6 と同じ差し替え文(「新しく決まった障害年金の10件のうち7件は、精神の障害です。」)をそのまま使えばよい。
