...
3. (2026-09-08 に解消: 記事のリンクを `/jitsurei?case=<id>` にし、/jitsurei がその id の載るページを描いて #id へスクロールするようにした。コミット `fix(jitsurei): ?case=id で該当事案のページを開く`。検証は docs/verification/jitsurei-case-2026-09-08/)**`/jitsurei#id` のアンカーは、ページ分割のため 1 ページ目に無い id がある**: r07-07_02 は 1 ページ目、r06-07_01 と r06-03_05 は 2 ページ目、r04_05-06_04 は 3 ページ目(`/jitsurei` は 12 件ずつ)。`/jitsurei#r04_05-06_04` を開いても 1 ページ目にその事案は無く、スクロールしない。直すなら `/jitsurei?page=3#id` にする(原稿のリンクを変える)か、`/jitsurei` 側で id 指定のとき該当ページへ回す。今回は原稿のとおりにしている。
