...
> **2026-09-04 取り下げ。6-2(「打ち切り」の語)以外は書かない。理由は `docs/claude-code-seo-aio-2-2026-09-04-instructions.md` §1。**
> 既存47本を読み直したところ受け皿は既にあり(6-1・6-4 は `shinsei-kikan` に、6-3 は `taishou-shoubyou-kyoukai` と
> `/byoki/tekiou-fuan` に)、新設すると共食いになる。本当の穴は「47本からハブ44ページへのリンクが0本」だった。
> 判断の記録として、下の本文はそのまま残す。
