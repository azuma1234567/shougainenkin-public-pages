...
import {
  ...
  type CircleSlot, type DateRow, type DigitsSlot, type MoushitateBlock, type PeriodRow, type TextSlot,
...
function Moushitate({ block, state, fontPt }: { block: MoushitateBlock; state: MoushitateshoState; fontPt: number }) {
  ...
  return (
    <>
      ...
      <Text slot={block.tel} value={state.seikyuusha.tel} fontPt={fontPt} />
      ...
      <Text slot={block.daihitsuTel} value={state.daihitsu?.tel ?? ""} fontPt={fontPt} />
    </>
  );
}
...
export function Sheet({ kind, state, cont, no, total }: SheetProps) {
  ...
  return (
    <div className={`mt-paper mt-${kind}`} data-sheet={kind}
      ...
      {kind === "main-front" && (
        <>
          <Digits slot={MAIN_FRONT.no} value={no} />
          <Digits slot={MAIN_FRONT.total} value={total} />
      ...
      {kind === "cont-front" && cont && (
        <>
          <Digits slot={CONT_FRONT.no} value={no} />
          <Digits slot={CONT_FRONT.total} value={total} />
