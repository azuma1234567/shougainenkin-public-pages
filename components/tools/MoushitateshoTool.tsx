...
const SSR:MoushitateshoState={version:3,byoumei:"",hatsubyou:"",shoshin:"",ninteibi:"",waku:[],back:{nintei:emptyBack(),genzai:emptyBack()},techou:null,techouList:[],seikyuusha:{name:"",address:"",tel:""},moushitateDate:"",daihitsu:null,seikyuuType:null,fontPt:10.5,updatedAt:""};
...
export default function MoushitateshoTool(){
 ...
 const go=(n:number)=>{setStep(Math.max(0,Math.min(8,n)));window.scrollTo({top:0,behavior:"smooth"})};
 ...
 step===1?<When state={state} patch={patch}/>:step===2?<Periods state={state} setWaku={setWaku} patch={patch}/>:step===3?<Period state={state} index={period} setWaku={setWaku} patch={patch}/>:step===4?<Life title="いまの生活のことを教えてください" value={state.back.genzai} formal={DAILY_LEVELS_GENZAI} change={v=>patch({back:{...state.back,genzai:v}})} state={state} patch={patch} notebook/>:step===5?<Retro state={state} patch={patch}/>:step===6?<Life title={`初診日から1年6か月たった頃(${recognition(state.shoshin)}ごろ)の生活のこと`} value={state.back.nintei} formal={DAILY_LEVELS_NINTEI} change={v=>patch({back:{...state.back,nintei:v}})} state={state} patch={patch}/>:step===7?<Seikyuusha state={state} patch={patch}/>:<Finish {...{state,noSave,setNoSave,exportJson,importJson,confirmDelete,setConfirmDelete}} onDelete={()=>{clearMoushitatesho();setState(emptyState());setHasDraft(false);setConfirmDelete(false)}}/>;
 return <div className="mt-tool mt-redesign">
  {step>0&&<header className="mt-flow-head"><div className="mt-progress-line" role="progressbar" aria-valuemin={1} aria-valuemax={9} aria-valuenow={step+1}><span style={{width:`${(step+1)/9*100}%`}}/></div><button className="mt-interrupt" onClick={()=>{alert("ここまでの内容は残っています。いつでも続きから始められます");window.location.href="/shinsei"}}>中断する</button></header>}
  <section className={`mt-panel${step===0?" mt-intro-panel":""}`}>{screen}</section>
  {step>0&&<><p className="mt-save-status" role="status">{noSave||!saveOk?"この端末では保存できません。閉じると消えます":"ここまでの内容は、この端末に残っています"}</p><footer className="mt-actions"><button onClick={back}>戻る</button>{step<8&&<button key={`${step}-${period}`} autoFocus className="mt-primary" onClick={next}>次へ</button>}</footer></>}
 </div>
}

function Intro(p:any){return <><h2 className="mt-intro-h2">申立書を、自分で書きたい</h2><p className="mt-intro-lead">病歴・就労状況等申立書は、発病から今までを期間に分けて書く書類です。<br/>ここでは、期間の区切り方から、各期間の書き方まで、順番に進めます。<br/>書いた内容は公式の様式に重ねて印刷できます。手書きで出すつもりの方も、期間の整理だけ使えます。</p><p className="mt-reassurance"><span>30分から。何日かに分けて大丈夫</span><span>書いたものはこの端末に残ります</span><span>サーバーには送りません。AIも使いません</span></p>
...
/* 発病日・初診日は年月日(記載要領)。日が分からないときは YYYY-MM で保存し、紙の日欄は空欄になる(§1-2)。 */
const hasDay=(v:string)=>/^\d{4}-\d{2}-\d{2}$/.test(v);
...
function DateField({label,help,value,onChange}:{label:string,help:string,value:string,onChange:(v:string)=>void}){
 /* 「日は分からない」は保存値の形(YYYY-MM)から復元する。空欄のときはチェックの状態を自前で持つ。 */
 const [unknown,setUnknown]=useState(!!value&&!hasDay(value));
 useEffect(()=>{if(value)setUnknown(!hasDay(value))},[value]);
 const toggle=(checked:boolean)=>{setUnknown(checked);if(checked)onChange(value?monthOf(value):"");else onChange("")};
 return <div className="mt-datefield"><Field label={label}>{unknown?<input type="month" value={monthOf(value)} onChange={e=>onChange(e.target.value)}/>:<input type="date" value={hasDay(value)?value:""} onChange={e=>onChange(e.target.value)}/>}</Field><label className="mt-check"><input type="checkbox" checked={unknown} onChange={e=>toggle(e.target.checked)}/>日は分からない(年月だけ書く。紙の日欄は空欄になります)</label><p className="mt-calm">{help}</p></div>}
function When({state,patch}:any){return <><h2>まず、3つだけ教えてください</h2><div className="mt-fields"><Field label="傷病名(診断書に書かれている名前)"><input value={state.byoumei} onChange={e=>patch({byoumei:e.target.value})}/></Field><Capacity slot={MAIN_FRONT.byoumei} text={state.byoumei} fontPt={state.fontPt}/><p className="mt-calm">請求する病気やけがが複数あるときは、傷病ごとに用紙を分けます。ここでは1つ分を書きます。</p>
 <DateField label="発病日(自覚症状が現れた日)" help="先天性のものは、症状を自覚した日か、検査で異常が見つかった日。生来性の知的障害は出生日です。日が分からなければ、チェックを入れて年月だけで進めます。" value={state.hatsubyou} onChange={v=>patch({hatsubyou:v})}/>
 <DateField label="初診日(その症状で初めて診療を受けた日)" help="受診状況等証明書の初診日と同じ日にします。生来性の知的障害は出生日です。日が分からなければ、チェックを入れて年月だけで進めます。" value={state.shoshin} onChange={v=>patch({shoshin:v,ninteibi:ninteibiFrom(v)})}/>
 </div><p className="mt-calm">思い出せないときは空欄のままで進めます。あとで直せます。</p></>}
...
function Life({title,value,formal,change,state,patch,notebook=false}:{title:string,value:BackSide,formal:readonly string[],change:(v:BackSide)=>void,state:MoushitateshoState,patch:(p:Partial<MoushitateshoState>)=>void,notebook?:boolean}){return <><h2>{title}</h2>
 ...
 <div className="mt-life">{DAILY_ITEMS.map((item,i)=><fieldset key={item}><legend>{QUESTIONS[i]}</legend><small>{item}</small><div>{PLAIN.map((plain,j)=><label key={plain}><input type="radio" name={`daily-${notebook?"now":"then"}-${i}`} checked={value.daily[i]===j+1} onChange={()=>change({...value,daily:{...value.daily,[i]:(j+1) as 1|2|3|4}})}/><span><strong>{plain}</strong><small>{formal[j]}</small></span></label>)}</div></fieldset>)}</div><div className="mt-fields"><Field label={notebook?"そのほかに、日常生活で不便に感じていること":"そのほかに、日常生活で不便に感じていたこと"}><textarea rows={3} value={value.sonota} onChange={e=>change({...value,sonota:e.target.value})}/></Field><Capacity slot={MAIN_BACK.sections[notebook?1:0].sonota} text={value.sonota} fontPt={state.fontPt}/></div>{notebook&&<fieldset className="mt-notebook"><legend>障害者手帳を持っていますか</legend><Choices options={["持っている","持っていない","申請中"]} selected={state.techou==="ari"?0:state.techou==="nashi"?1:state.techou==="shinsei"?2:-1} choose={i=>patch({techou:["ari","nashi","shinsei"][i] as any,techouList:i===0&&state.techouList.length===0?[emptyTechou()]:state.techouList})}/>{state.techou==="ari"&&<TechouFields state={state} patch={patch}/>}</fieldset>}</>}
