...
export default function ShinseiRestyled() {
  ...
  return (
    <div className="platform shinsei-page">
      ...
      <header className="p-page-hero shinsei-hero"><div className="p-container shinsei-reading-width"><Breadcrumb items={[{ href: "/", label: "トップ" }, { label: "申請の流れ" }]} currentPath="/shinsei" /><h1>申請の流れ — 8つのステップ</h1><p className="p-page-intro">初診日の確認から結果が届くまで。全体の地図を先に持つと、いま自分がどこにいるかで迷いにくくなります。1ステップずつ、必要なことだけを載せています。</p><nav className="shinsei-stepper" aria-label="8つのステップ">{STEPS.map((step, index) => <a href={`#${step.id}`} key={step.id}><span>{index + 1}</span><b>{step.short}</b></a>)}</nav></div></header>
