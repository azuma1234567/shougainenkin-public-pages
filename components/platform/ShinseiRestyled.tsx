...
export default function ShinseiRestyled() {
  ...
  return (
    <div className="platform shinsei-page">
      ...
      <header className="p-page-hero shinsei-hero"><div className="p-container shinsei-reading-width"><Breadcrumb items={[{ href: "/", label: "トップ" }, { label: "申請の流れ" }]} currentPath="/shinsei" /><h1>申請の流れ — 8つのステップ</h1><p className="p-page-intro">初診日の確認から結果が届くまで。全体の地図を先に持つと、いま自分がどこにいるかで迷いにくくなります。1ステップずつ、必要なことだけを載せています。</p><StepFlow tools={false} /></div></header>
      <div className="p-container shinsei-reading-width shinsei-content">
        ...
        <section className="shinsei-app" aria-labelledby="app-heading"><span className="p-label">申請準備の味方</span><h2 id="app-heading">申請の現在地と、次の一歩を、いつでも手元に</h2><p>「障害年金申請サポート」は、申請を代行するアプリではありません。日々の記録・診察メモ・申立書の準備を、自分のペースで進めるための味方です。</p><ul>{APP_POINTS.map((item) => <li key={item}><CheckIcon size={15} />{item}</li>)}</ul><AppStoreBadge href={APP_STORE_URL} /></section>
